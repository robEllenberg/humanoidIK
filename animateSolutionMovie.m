function animateSolutionMovie(Angles,~,Frequency,IKSolver,h,walkarea)
            ...
            for k=frames
                ...
                renderFrame(IKSolver,Angles(:,k),[],cmap(k,:),h_leg,h_rot,walkarea);
                drawnow();
                    frame = getframe(gca);
...
function [h_leg,h_rot]=renderFrame(IKSolver,angles,~,color,h_leg,h_rot,walkarea)
            [leftLeg,rightLeg,leftArm,rightArm]=IKSolver.Points(angles);
            leftLeg=cumsum(leftLeg,2);
            rightLeg=cumsum(rightLeg,2);
            leftArm=cumsum(leftArm,2);
            rightArm=cumsum(rightArm,2);
            [R_LeftLeg,R_RightLeg,R_LeftArm,R_RightArm]=IKSolver.Csys(angles);
